...
$exEligibleOnly = !empty($_POST['exEligibleOnly']) ? $_POST['exEligibleOnly'] : false;
...
if (!$noGyms || !$noRaids) {
    if ($d["lastgyms"] == "true") {
        if ($lastgyms != "true") {
            $d["gyms"] = $scanner->get_gyms($swLat, $swLng, $neLat, $neLng, $exEligibleOnly);
        } else {
            if ($newarea) {
                $d["gyms"] = $scanner->get_gyms($swLat, $swLng, $neLat, $neLng, $exEligibleOnly, 0, $oSwLat, $oSwLng, $oNeLat, $oNeLng);
            } else {
                $d["gyms"] = $scanner->get_gyms($swLat, $swLng, $neLat, $neLng, $exEligibleOnly, $timestamp);
            }
        }
    }
}
